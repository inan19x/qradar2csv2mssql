...
def get_daily_log(api_client, time_period):
    ...
    create_log(get_json_log(api_client, get_query1(time_period)), 'T_Query1_{}.csv'.format(day_filename))
...
#Your AQL query goes here:
def get_query1(time_period):
    query_expression = """ 
        SELECT QIDName(qid) as EventName, COUNT(*) as Total
        FROM events
        GROUP BY EventName
        ORDER BY Total DESC
        START '{0} 00:00:00' STOP '{1} 00:00:00'
        """.format(time_period['day'], time_period['day_plus_one'])
    return query_expression


def create_log(json_data, filename):
	output_path = os.path.realpath('csvdata')
	keylist = []
